...
def pre_analysis_bid():
    ...
    try:
        ...
        你是服务于湖北恩施水利配套制造企业的资深招投标文件分析师。该企业长期承接三峡集团及其水利工程供应链相关配套业务，产品和服务范围包括涡片、螺母、紧固件、金属结构件、设备配套加工、质量检验、交付保障与现场配合。请根据以下招标书内容，提炼出完整信息，并严格按照下面的JSON格式返回你的分析结果，不要有任何多余的解释，只返回以下json内容。
        {{
...
def chapter_analysis_bid():
    ...
    try:
        ...
        你是服务于湖北恩施水利配套制造企业的资深招投标文件结构分析师，熟悉三峡集团及水利工程供应链投标文件的技术、商务、资质、质量、安全、交付和售后响应要求。请根据以下招标书内容，输出投标书其他响应文件的格式章节内容（除封面章节），并严格按照下面的JSON格式返回你的分析结果，不要有任何多余的解释。
        {{
            "chapter_format":"..."
        }}
...
def chapter_design():
    ...
    chapter_design_prompt = (
        f"你是服务于湖北恩施水利配套制造企业的资深投标文件目录结构设计专家，熟悉三峡集团及水利工程配套产品供应链投标规范。请根据以下信息，整理出最终的标书章节结构：\n\n"
        f"必须包含的文件和材料：{bidding_requirements}\n"
        f"招标书内容总结：{bidding_summary}\n"
        f"招标书具体要求和评分标准：{bidding_meta}\n"
        f"投标书章节大纲：{directory_structure}\n\n"
        "基于以上招标文件要求、企业水利配套制造经验和三峡集团供应链响应习惯，请补充章节的子节目录，确保投标文件完整、严谨、可执行且符合要求。\n"
        "要求：\n"

...
def list_knowledge_bases():
    ...
    try:
        rows = conn.execute(
            """
            SELECT id, kb_name AS kbName, kb_type AS kbType, description, visibility_scope AS visibilityScope,
...
def search_knowledge():
    ...
    try:
        ...
        return jsonify({"items": pack["items"], "degraded": pack["degraded"], "degradedReason": pack["degraded_reason"], "fallbackUsed": pack["fallback_used"]})
    except AuthenticationError:
